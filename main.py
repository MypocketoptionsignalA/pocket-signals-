...
async def btn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('pair_'):
        ...
        logger.info(f"✅ Pair selected: {pair} by user {user_id}")
        ...
        logger.info(f"✅ Timeframe buttons shown to user {user_id}")
    
    # Handle timeframe selection
    elif data.startswith('time_'):
        logger.info(f"⏱ TIMEFRAME BUTTON CLICKED by user {user_id}: {data}")
        timeframe = data.replace('time_', '')
        state.selected_timeframe[user_id] = timeframe
        logger.info(f"✅ Timeframe saved: {timeframe}")
        
        # Stop any existing signals
        cid = query.message.chat_id
        logger.info(f"📍 Chat ID: {cid}")
        current_jobs = context.job_queue.get_jobs_by_name(f'signal_{user_id}')
        for job in current_jobs:
            job.schedule_removal()
        ...
        logger.info(f"🟢 ACTIVE SET TO TRUE for user {user_id}")
        ...
        test_signal = f"✅ <b>SIGNALS ACTIVATED!</b>\n\n📊 Pair: {pair}\n⏱ Timeframe: {timeframe}\n\n🚀 First signal in 3 seconds..."
        await query.edit_message_text(test_signal, parse_mode='HTML')
        logger.info(f"✅ Activation message sent!")
        
        # Send signals for selected timeframe
        interval = TIMEFRAMES[timeframe]
        logger.info(f"⏱ Interval: {interval} seconds")
        
        # Schedule first signal after 3 seconds
        logger.info(f"🚀 Scheduling first signal in 3 seconds...")
        context.job_queue.run_once(
            lambda c: send_signal_now(c, cid, user_id),
            when=3,
            name=f'first_signal_{user_id}'
        )
        logger.info(f"✅ First signal scheduled!")
        
        # Schedule recurring signals
        logger.info(f"📅 Scheduling recurring signals every {interval} seconds...")
        ...
        logger.info(f"✅✅✅ ALL SIGNALS SCHEDULED! User {user_id} should now receive signals!")
            name=f'first_signal_{user_id}'
        )
        ...
        logger.info(f"✅ Started signals for user {user_id} - {pair} - {timeframe}")
